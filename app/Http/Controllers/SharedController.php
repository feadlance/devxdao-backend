...
class SharedController extends Controller
{
			...
			if ($profile) {
				...
				if ($signObject) {
					...
					$informalVote = Vote::where('proposal_id',  $proposal->id)
						...
						->first();
					if ($informalVote && $settings['autostart_formal_votes'] == 'yes') {
				...
				$proposal->grant_hellosign_form = $filepath;
				$proposal->save();
				$signObject = SignatureGrant::where('proposal_id', $proposal->id)->where('signed', 0)->first();
				if ($signObject) {
					$signObject->signed = 1;
					$signObject->save();
				}
	...
	public function startFormalVoting(Request $request)
	{
		...
		if ($user) {
			...
			// Proposal Check
			if (!$proposal || $proposal->type != "simple") {
				return [
					'success' => false,
	...
	public function getGrants(Request $request)
	{
		...
		if ($user->hasRole('admin')) {
			$proposals = FinalGrant::with(['proposal', 'proposal.user', 'proposal.milestones', 'user', 'signtureGrants']);
			if ($status == 'pending') {
